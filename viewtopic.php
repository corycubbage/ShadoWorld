...
debug_to_console ("Loading viewtopic.php");
...
*/
//my code added USER_VARIABLES_TABLE archi11
$sql_ary = array( 'SELECT' => 'u.*, z.friend, z.foe, p.*,uv.*',
    ...
    'LEFT_JOIN' => array(
        ...
        array(
            'FROM' => array(USER_VARIABLES_TABLE => 'uv'),
...
while ($row = $db->sql_fetchrow($result))
{
	...
	if ($row['post_attachment'] && $config['allow_attachments'])
	{
        ...
        //debug_to_console("|- Setting rowset_data.");
	$rowset_data = array(
		...
		'selected_current_hit' => $row['selected_current_hit'],
                ...
                'gear' => $row['gear'],
                'hero_point' => $row['hero_point'],
                'quick_stats' => $row['quick_stats'],
                'positive_condition_description' => $row['positive_condition_description'],
                'negative_concdition_description' => $row['negative_concdition_description'],
                'abilities_description' => $row['abilities_description'],
                'gear_description' => $row['gear_description'],
                );
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$message = generate_text_for_display($row['post_text'], $row['bbcode_uid'], $row['bbcode_bitfield'], $parse_flags, true);
     ...
     //archi11 code start
    //current and maximum hit variable
    $PLAYERINFO  = censor_text($row['PLAYERINFO']);
    $CLASS_INFO = censor_text($row['CLASS_INFO']);
    //$AC = censor_text($row['AC']);
    $selected_current_hit = censor_text($row['selected_current_hit']);
    $selected_maximum_hit = censor_text($row['selected_maximum_hit']);
    $seleted_non_lethal = censor_text($row['seleted_non_lethal']);
...
$Decoded_SAVES_count = count($DecodedSAVES);
    ...
    if ($DecodedSAVES) {
        $FORT = $DecodedSAVES[0];          //Fort Save
        $REFLEX = $DecodedSAVES[1];         //Reflex Save
        $WILL = $DecodedSAVES[2];        //Will Save
            }
    else {
        $FORT = 0;
        $REFLEX = 0;
...
$Decoded_RESISTIMMUNITY_count = count($DecodedRESISTIMMUNITY);
    ...
    if ($DecodedRESISTIMMUNITY) {
        $RESIST = $DecodedRESISTIMMUNITY[0];          //Resistance
        $IMMUNITY = $DecodedRESISTIMMUNITY[1];         //Immunity
            }
    else {
        $RESIST = '';
        $IMMUNITY = '';
    ...
    // seleted bad condition
    $sbc = json_decode($row['seleted_bad_condition'], true);
    $nc = json_decode($row['negative_concdition_description'], true);
    //debug_to_console("|- sbc: ." . $sbc);
    //debug_to_console("|- nc: " . $nc);

    $seleted_bad_condition = '';                                                //CVC - 12/02/15 - Fixes Bug #3 - Conditions stacking in the profile area.
    if (count($sbc) > 1) {
        //$seleted_bad_condition = '';
        for ($j = 0; $j < count($sbc); $j++) {
            if ($j != (count($sbc) - 1)) {
          	$query = "SELECT * FROM `phpbb_condition`";
			$result = mysql_query($query);
			while($rows = mysql_fetch_assoc($result))
			{
				//$condition[] = array($row['bad_condition'], $row['bad_desc']);
				if($rows['bad_condition'] == $sbc[$j])
				{
					$seleted_bad_condition .="<label id='hover1' for='" . $sbc[$j] . "' title='" . $rows['bad_desc'] . "' >" . $sbc[$j]."," . "</label>";
				}


			}
               // $seleted_bad_condition .="<label id='hover1' for='" . $sbc[$j] . "' title='" . $nc[$sbc[$j]] . "'>" . $sbc[$j] . "</label>,&nbsp;";
            } else {
	...
	$query = "SELECT * FROM `phpbb_condition`";
		...
		{
			//$condition[] = array($row['bad_condition'], $row['bad_desc']);
			if($rows['bad_condition'] == $sbc[0])

			{
				$seleted_bad_condition .="<label id='hover1' for='" . $sbc[0] . "' title='" . $rows['bad_desc'] . "' >" . $sbc[0] . "</label>";
			}
		}
	
     // $seleted_bad_condition .="<label id='hover1' for='" . $sbc[0] . "' title='" . $nc[$sbc[0]] . "' >" . $sbc[0] . "</label>";
    } elseif ($sbc == null) {
        $seleted_bad_condition = '';
    }

    //seleted goog condition 

    $sgc = json_decode($row['seleted_good_condition'], true);
    $pc = json_decode($row['positive_condition_description'], true);

    $seleted_good_condition = '';                                               //CVC - 12/02/15 - Fixes Bug #3 - Conditions stacking in the profile area.
    if (count($sgc) > 1) {
        //$seleted_good_condition = '';
        for ($k = 0; $k < count($sgc); $k++) {
            if ($k != (count($sgc) - 1)) {
            
                $seleted_good_condition .="<label id='hover' for='" . $sgc[$k] . "' title='" . $pc[$sgc[$k]] . "'>" . $sgc[$k] . "</label>,&nbsp;";
            } else {
        ...
        $seleted_good_condition .="<label id='hover' for='" . $sgc[0] . "' title='" . $pc[$sgc[0]] . "' >" . $sgc[0] . "</label>";
    } elseif ($sbc == null) {
        $seleted_good_condition = '';
    }
    ...
    if (count($level) > 1 && ($level != null )) {
        ...
        for ($m = 0; $m < count($level); $m++) {
            if ($m != (count($level) - 1)) {
                $levels .=$l . "" . $level[$m] . ": " . $level_min[$m] . "/" . $level_max[$m] . ",&nbsp";
            } else {
                $levels .=$l . $level[$m] . ": " . $level_min[$m] . "/" . $level_max[$m];
            }
        }
    ...
    if (count($ability_name) > 1 && ($ability_name != null)) {
        ...
        for ($a = 0; $a < count($ability_name); $a++) {
            if ($a != (count($ability_name) - 1)) {
                  $ability .="<label id='hover' for='" . $ability_name[$a] . "' title='" . $ad[$ability_name[$a]] . "' >" . $ability_name[$a] . ": " . $min_ability[$a] . "/" . $max_ability[$a] . "</label>";
            } else {
                $ability .="<label id='hover' for='" . $ability_name[$a] . "' title='" . $ad[$ability_name[$a]] . "' >" . $ability_name[$a] . ": " . $min_ability[$a] . "/" . $max_ability[$a] . "</label>";
            }
        }
    } elseif (($ability_name[0] != "") && (count($ability_name) == 1)) {
         $ability .="<label id='hover' for='" . $ability_name[0] . "' title='" . $ad[$ability_name[0]] . "' >" . $ability_name[0] . ": " . $min_ability[0] . "/" . $max_ability[0] . "</label>";
    } else {
        $ability = '';
    }
    ...
    //selected gear 
    $gear = json_decode($row['gear'], true);
    $gc = json_decode($row['gear_description'], true);

    if (count($gear) > 1 && ($gear != null)) {
        $geardata = 'Gear : ';
        for ($g = 0; $g < count($gear); $g++) {
            if ($g != (count($gear) - 1)) {
         
                $geardata .="<label id='hover' for='" . $gear[$g] . "' title='" . $gc[$gear[$g]] . "' >".$gear[$g].",</label>";
            } else {
              
                $geardata .="<label id='hover' for='" . $gear[$g] . "' title='" . $gc[$gear[$g]] . "' >" . $gear[$g] . "</label>";
            }
        }
    } elseif (count($gear) == 1 && ($gear != null)) {
            $geardata .="<label id='hover' for='" . $gear[0] . "' title='" . $gc[$gear[0]] . "' >" . $gear[0] . "</label>";
    } else {
       
        $geardata .="<label id='hover' for='null' title='no description' >dd</label>";
    }
    ...
    $quick_stats = $row['quick_stats'];
    $hero_point = $row['hero_point'];
    //$ability for the ability , min and max abilities     
    //archi11 code ends
	...
	*/
	...
	//
                //debug_to_console ('|- SELECTED_CURRENT_HIT: '. $selected_current_hit);
                //debug_to_console ('|- SELECTED_MAXIMUM_HIT' . $selected_maximum_hit);
                //debug_to_console ('|- SELECTED_BAD_CONDITION: '. $seleted_bad_condition);
                //debug_to_console ('|- SELECTED_GOOD_CONDITION: ' . $seleted_good_condition);
	...
	$post_row = array(
		...
		//archi11 code start      
                'AC' => $AC,
                'TAC' => $TAC,                                                              //CVC - 12/04/15 (Added)
                'FFAC' => $FFAC,                                                            //CVC - 12/04/15 (Added)
                'FORT' => $FORT,                                                            //CVC - 12/04/15 (Added)
                'REFLEX' => $REFLEX,                                                        //CVC - 12/04/15 (Added)
		...
		'CLASS_INFO' => $CLASS_INFO,
                ...
                'SELECTED_NON_LETHAL' => $seleted_non_lethal,
                'SELECTED_BAD_CONDITION' => $seleted_bad_condition,
                'SELECTED_GOOD_CONDITION' => $seleted_good_condition,
                'LEVELS' => $levels,
                'ABILITY' => $ability,
                'GEAR' => $geardata,
                ...
                /*foreach ($post_row as $k => $v) {
                    echo nl2br ("\$post_row[$k] => string($v).\n");
                }*/
