...
function getGroupMemberStats($forum_id)
{
    //debug_to_console ("Calling getGroupStats for forum_isd: " . getGroupMemberStats); 
    global $db;
    
	$data = array();
	$select = "select distinct uv.*, u.username from ".USERS_TABLE." u inner join ".ACL_GROUPS_TABLE." g on u.group_id = g.group_id inner join " . USER_VARIABLES_TABLE . " uv on u.user_id = uv.user_id where u.AccountType = 0 AND g.forum_id = ".$forum_id;
...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true, $update_search_index = true)
{
	...
	*/
	...
	//$poster_id = ($mode == 'edit') ? $data['poster_id'] : (int) $user->data['user_id'];  // CVC 11/26/15
        $poster_id = ($mode == 'edit') ? $data['poster_id'] : (int) $data['poster_id'];
	...
	switch ($post_mode)
	{
		...
		case 'reply':
			$sql_data[POSTS_TABLE]['sql'] = array(
				'forum_id'			=> $data['forum_id'],
                                //'poster_id'			=> (int) $user->data['user_id'],	 // CVC112615
				...
				'enable_sig'		=> $data['enable_sig'],
                                ...
                                //'post_username'		=> (!$user->data['is_registered']) ? $poster_name : '',     /CVC112715
                                'post_username'		=> $data['poster_name'],  //(!$user->data['is_registered']) ? $data['poster_name'] : '',  CVC 112715
		...
		case 'edit_topic':
			...
			$sql_data[POSTS_TABLE]['sql'] = array_merge($sql_data[POSTS_TABLE]['sql'], array(
				...
				'enable_magic_url'	=> $data['enable_urls'],
				'enable_sig'		=> $data['enable_sig'],
				//'post_username'	=> ($username && $data['poster_id'] == ANONYMOUS) ? $username : '',             //CVC 112715
                                'post_username'		=> ($username && $data['poster_id'] == ANONYMOUS) ? $data['poster_name'] : '',
				'post_subject'		=> $subject,
				'post_checksum'		=> $data['message_md5'],
				'post_attachment'	=> (!empty($data['attachment_data'])) ? 1 : 0,
				'bbcode_bitfield'	=> $data['bbcode_bitfield'],
			...
			if ($update_message)
			{
				$sql_data[POSTS_TABLE]['sql']['post_text'] = $data['message'];
                                //echo nl2br("Updating Message in Functions_Posting!\n");
	...
	switch ($post_mode)
	{
		case 'post':
			$sql_data[TOPICS_TABLE]['sql'] = array(
				//'topic_poster'				=> (int) $user->data['user_id'],  //CVC112615
                                'topic_poster' => $data['poster_id'],       //CVC 112615
				...
				'topic_visibility'			=> $post_visibility,
                                'topic_delete_user'			=> ($post_visibility != ITEM_APPROVED) ? (int) $data['poster_id'] : 0,	
                                //'topic_delete_user'			=> ($post_visibility != ITEM_APPROVED) ? (int) $user->data['user_id'] : 0,   //cvc112615
				'topic_title'				=> $subject,
				'topic_first_poster_name'               => $data['poster_name'],
                                //'topic_first_poster_name'	=> (!$user->data['is_registered'] && $username) ? $username : (($user->data['user_id'] != ANONYMOUS) ? $user->data['username'] : ''),   //CVC 112615
				//'topic_first_poster_colour'	=> $user->data['user_colour'],  // CVC112615
                                'topic_first_poster_colour'	=> $data['user_colour'],
		...
		case 'edit_first_post':
			...
			$sql_data[TOPICS_TABLE]['sql'] = array(
				...
				'icon_id'				=> $data['icon_id'],
				'topic_title'				=> $subject,
                                'topic_first_poster_name'               => $data['poster_name'],        //$poster_name,	 CVC112615
                                //'topic_first_poster_name'             => $username,           //CVC112615
				'topic_type'				=> $topic_type,
				'topic_time_limit'			=> ($topic_type == POST_STICKY || $topic_type == POST_ANNOUNCE) ? ($data['topic_time_limit'] * 86400) : 0,
	...
	if ($post_mode == 'post' || $post_mode == 'reply')
	{
		if ($post_mode == 'reply')
		{
         ...
         $data_user = array(
            ...
            'selected_current_hit' => $data['selected_current_hit'],
            'selected_maximum_hit' => $data['selected_maximum_hit'],
            'seleted_non_lethal' => $data['seleted_non_lethal'],
            'seleted_bad_condition' => $data['seleted_bad_condition'],
            'seleted_good_condition' => $data['seleted_good_condition'],
            'level' => $data['level'],
            'level_min' => $data['level_min'],
            'level_max' => $data['level_max'],
            'ability_name' => $data['ability_name'],
            'min_ability' => $data['min_ability'],
            'max_ability' => $data['max_ability'],
            'gear' => $data['gear'],
            'quick_stats' => $data['quick_stats'],
            'hero_point' => $data['hero_point'],
            'quick_skill' => $data['quick_skill'],
            'spell' => $data['spell'],
        );
        //archi11 for the post table end

        $sel = 'SELECT * FROM ' . USER_VARIABLES_TABLE;
        $result1 = $db->sql_query($sel);
        $count = 0;
        while ($row = $db->sql_fetchrow($result1)) {
            $id[] = $row['user_id'];
        }
        array_unique($id);
        if (in_array($data['poster_id'], $id)) {
            $sel_weapon = 'SELECT * FROM ' . USER_VARIABLES_TABLE;
            $result_weapon = $db->sql_query($sel_weapon);
            $row_weapon = $db->sql_fetchrow($result_weapon);
            
          $data_user['weapon_name']=$row_weapon['weapon_name']; 
          $data_user['weapon_field']=$row_weapon['weapon_field']; 
            
         $sql = 'UPDATE ' . USER_VARIABLES_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $data_user) . ' WHERE user_id = ' . $data['poster_id'];
         $db->sql_query($sql);
        } else {
           $sql = 'INSERT INTO ' . USER_VARIABLES_TABLE . ' ' . $db->sql_build_array('INSERT', $data_user);
            $db->sql_query($sql);
		...
		if ($post_mode == 'post' || $post_visibility == ITEM_APPROVED)
		{
			$sql_data[TOPICS_TABLE]['sql'] = array(
				...
				'topic_last_poster_id'		=> $sql_data[POSTS_TABLE]['sql']['poster_id'],
                                ...
                                'topic_last_poster_name'	=> ($data['poster_id'] == ANONYMOUS) ? $sql_data[POSTS_TABLE]['sql']['poster_name'] : $data['poster_name'],             //11C2715
				//'topic_last_poster_colour'	=> $user->data['user_colour'],
                                'topic_last_poster_colour'	=> $data['user_colour'],
...
//                        $sql_data[FORUMS_TABLE]['stat'][] = 'forum_last_poster_id = ' . (int) $user->data['user_id'];
			$sql_data[FORUMS_TABLE]['stat'][] = "forum_last_poster_name = '" . $db->sql_escape((!$user->data['is_registered'] && $poster_name) ? $poster_name : (($data['poster_id'] != ANONYMOUS) ? $data['poster_name'] : '')) . "'";
                        //$sql_data[FORUMS_TABLE]['stat'][] = "forum_last_poster_name = '" . $db->sql_escape((!$user->data['is_registered'] && $username) ? $username : (($user->data['user_id'] != ANONYMOUS) ? $user->data['username'] : '')) . "'";
			//$sql_data[FORUMS_TABLE]['stat'][] = "forum_last_poster_colour = '" . $db->sql_escape($user->data['user_colour']) . "'";  //CVC112615
                        $sql_data[FORUMS_TABLE]['stat'][] = "forum_last_poster_colour = '" . $db->sql_escape($data['user_colour']) . "'";
		}
		...
		if (sizeof($orphan_rows))
		{
			$sql = 'SELECT attach_id, filesize, physical_filename
				...
				WHERE ' . $db->sql_in_set('attach_id', array_keys($orphan_rows)) . '
					AND is_orphan = 1
					AND poster_id = ' . $data['poster_id'];
                                        //AND poster_id = ' . $user->data['user_id'];   //CVC112615
			$result = $db->sql_query($sql);
	...
	if ($draft_id)
	{
		$sql = 'DELETE FROM ' . DRAFTS_TABLE . "
			WHERE draft_id = $draft_id
				AND user_id = {$data['poster_id']}";
                                //AND user_id = {$user->data['user_id']}";      //CVC112615
		$db->sql_query($sql);
	}
	...
	if ($user->data['user_id'] == $poster_id)
        if ($data['poster_id'] == $poster_id)                   //CVC112615
	{
		if (!$data['notify_set'] && $data['notify'])
		{
			$sql = 'INSERT INTO ' . TOPICS_WATCH_TABLE . ' (user_id, topic_id)
				VALUES (' . $data['poster_id'] . ', ' . $data['topic_id'] . ')';
                                //VALUES (' . $user->data['user_id'] . ', ' . $data['topic_id'] . ')';  //CVC112615
			$db->sql_query($sql);
		}
		else if (($config['email_enable'] || $config['jab_enable']) && $data['notify_set'] && !$data['notify'])
		{
			$sql = 'DELETE FROM ' . TOPICS_WATCH_TABLE . '
				WHERE user_id = ' . $data['poster_id'] . ' 
					AND topic_id = ' . $data['topic_id'];
			$db->sql_query($sql);
                        // WHERE user_id = ' . $user->data['user_id'] . '   //CVC 112615
		}
	}
	...
	if ($config['load_db_lastread'] && $user->data['is_registered'])
	{
		...
		// WHERE user_id = ' . $user->data['user_id'] . '  //CVC112615
                $result = $db->sql_query($sql);
	...
	$username = ($username !== '' || !$user->data['is_registered']) ? $poster_name : $data['poster_name'];
        //$username = ($username !== '' || !$user->data['is_registered']) ? $username : $user->data['username'];        //CVC112615
