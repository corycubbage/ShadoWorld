...
//archi11 code
function request_var_condition() {
    debug_to_console("* Calling request_var_condition");
    $array = array("negative_condition" => request_var('negative_condition',''), "positive_condition" => request_var('positive_condition',''), "gear" => request_var('gear',''));
    return $array;
...
function set_for_new_variables() {
    global $db;
    if ((@$_REQUEST['config']['new_current_hit'] != '')) {
        if (@$_REQUEST['config']['new_current_hit'] != 0) {
            $sql = 'SELECT current FROM ' . VARIABLES_TABLE;
            $result = $db->sql_query($sql);
            $re = $db->sql_fetchrow($result);
            $re1 = json_decode($re['current'], true);
            $crt = array($_REQUEST['config']['new_current_hit']);

            if (!(in_array($_REQUEST['config']['new_current_hit'], $re1))) {
                $a = array_merge($re1, $crt);
                $re3 = json_encode($a, JSON_FORCE_OBJECT);
                $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET current = '" . $re3 . "'";
                $db->sql_query($sql1);
            }
        }
    }

    if (@$_REQUEST['config']['new_maximum_hit'] != '') {
        if (@$_REQUEST['config']['new_maximum_hit'] != 0) {
            $sql = 'SELECT maximum FROM ' . VARIABLES_TABLE;
            $result = $db->sql_query($sql);
            $re = $db->sql_fetchrow($result);
            $re1 = json_decode($re['maximum'], true);
            $crt = array($_REQUEST['config']['new_maximum_hit']);
            if (!(in_array($_REQUEST['config']['new_maximum_hit'], $re1))) {
                $a = array_merge($re1, $crt);
                $re3 = json_encode($a, JSON_FORCE_OBJECT);
                $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET maximum = '" . $re3 . "'";
                $db->sql_query($sql1);
            }
        }
    }
...
function set_good_bad_new_condition() {
    global $db;

    if ((@$_REQUEST['config']['new_good_condition'] != '')) {

        $sql = 'SELECT positive_condition FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['positive_condition'], true);
        $crt = array($_REQUEST['config']['new_good_condition']);

        if (!(in_array($_REQUEST['config']['new_good_condition'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET positive_condition = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['config']['new_bad_condition'] != '')) {

        $sql = 'SELECT negative_condition FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['negative_condition'], true);
        $crt = array($_REQUEST['config']['new_bad_condition']);

        if (!(in_array($_REQUEST['config']['new_bad_condition'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET negative_condition = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }

    if ((@$_REQUEST['config']['new_spell'] != '')) {

        $sql = 'SELECT spell FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['spell'], true);
        $crt = array($_REQUEST['config']['new_spell']);

        if (!(in_array($_REQUEST['config']['new_spell'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET spell = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
}

function set_new_level_ability() {
    global $db;
    if ((@$_REQUEST['config']['new_level'] != '')) {

        $sql = 'SELECT level FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['level'], true);
        $crt = array($_REQUEST['config']['new_level']);

        if (!(in_array($_REQUEST['config']['new_level'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET level = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['config']['new_minimum_level'] != '')) {

        $sql = 'SELECT level_min FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['level_min'], true);
        $crt = array($_REQUEST['config']['new_minimum_level']);

        if (!(in_array($_REQUEST['config']['new_minimum_level'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET level_min = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['config']['new_maximum_level'] != '')) {

        $sql = 'SELECT level_max FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['level_max'], true);
        $crt = array($_REQUEST['config']['new_maximum_level']);

        if (!(in_array($_REQUEST['config']['new_maximum_level'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET level_max = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['config']['new_minimum_ability'] != '')) {

        $sql = 'SELECT min_ability FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['min_ability'], true);
        $crt = array($_REQUEST['config']['new_minimum_ability']);

        if (!(in_array($_REQUEST['config']['new_minimum_ability'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET min_ability = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['config']['new_maximum_ability'] != '')) {

        $sql = 'SELECT max_ability FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['max_ability'], true);
        $crt = array($_REQUEST['config']['new_maximum_ability']);

        if (!(in_array($_REQUEST['config']['new_maximum_ability'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET max_ability = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['config']['new_gear'] != '')) {

        $sql = 'SELECT gear FROM ' . VARIABLES_TABLE;
        $result = $db->sql_query($sql);
        $re = $db->sql_fetchrow($result);
        $re1 = json_decode($re['gear'], true);
        $crt = array($_REQUEST['config']['new_gear']);

        if (!(in_array($_REQUEST['config']['new_gear'], $re1))) {
            $a = array_merge($re1, $crt);
            $re3 = json_encode($a, JSON_FORCE_OBJECT);
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET gear = '" . $re3 . "'";
            $db->sql_query($sql1);
        }
    }
}

function set_config_for_change_variables() {
    global $db;

    $sql = 'SELECT * FROM ' . VARIABLES_TABLE;
    $result = $db->sql_query($sql);
    $current = array();
    $maximum = array();
    ...
    if ((@$_REQUEST['select_curren_hit'] != '') and ( @$_REQUEST['change_current_hit'] != '') and ( is_numeric($_REQUEST['change_current_hit']))) {

        $ch = json_decode($current, true);
        if (!in_array(@$_REQUEST['change_current_hit'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['select_curren_hit'] => $_REQUEST['change_current_hit']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET current = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['select_maximum_hit'] != '') and ( @$_REQUEST['change_maximum_hit'] != '') and ( is_numeric($_REQUEST['change_maximum_hit']))) {
        $ch = json_decode($maximum, true);
        if (!in_array(@$_REQUEST['change_maximum_hit'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['select_maximum_hit'] => $_REQUEST['change_maximum_hit']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET maximum = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
}

function set_chnage_good_bad_condition() {
    global $db;
    $sql = 'SELECT * FROM ' . VARIABLES_TABLE;
    $result = $db->sql_query($sql);
    $good = array();
    ...
    while ($row = $db->sql_fetchrow($result)) {
        $good = $row['positive_condition'];
        $bad = $row['negative_condition'];
    }

    if ((@$_REQUEST['selected_positive_condition'] != '') and ( @$_REQUEST['change_good_condition'] != '') and ( !is_numeric($_REQUEST['change_good_condition']))) {
        $ch = json_decode($good, true);
        if (!in_array(@$_REQUEST['change_good_condition'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_positive_condition'] => $_REQUEST['change_good_condition']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET positive_condition = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['selected_negative_condition'] != '') and ( @$_REQUEST['change_negative_condition'] != '') and ( is_numeric($_REQUEST['change_negative_condition']))) {
        $ch = json_decode($bad, true);
        if (!in_array(@$_REQUEST['change_negative_condition'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_negative_condition'] => $_REQUEST['change_negative_condition']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET negative_condition = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
}

function set_chnage_levels() {
    global $db;
    $sql = 'SELECT * FROM ' . VARIABLES_TABLE;
    $result = $db->sql_query($sql);
    ...
    if ((@$_REQUEST['selected_level'] != '') and ( @$_REQUEST['change_level'] != '') and ( is_numeric($_REQUEST['change_level']))) {
        $ch = json_decode($level, true);
        sort($ch);

        if (!in_array(@$_REQUEST['change_level'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_level'] => $_REQUEST['change_level']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET level = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['selected_level_min'] != '') and ( @$_REQUEST['change_level_min'] != '') and ( is_numeric($_REQUEST['change_level_min']))) {
        $ch = json_decode($level_min, true);
        if (!in_array(@$_REQUEST['change_level_min'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_level_min'] => $_REQUEST['change_level_min']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET level_min = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['selected_level_max'] != '') and ( @$_REQUEST['change_level_max'] != '') and ( is_numeric($_REQUEST['change_level_max']))) {
        $ch = json_decode($level_max, true);
        if (!in_array(@$_REQUEST['change_level_max'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_level_max'] => $_REQUEST['change_level_max']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET level_max = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
}

function set_chnage_ability() {
    global $db;
    $sql = 'SELECT * FROM ' . VARIABLES_TABLE;
    $result = $db->sql_query($sql);
    $good = array();
    ...
    if ((@$_REQUEST['selected_min_ability'] != '') and ( @$_REQUEST['change_min_ability'] != '') and ( is_numeric($_REQUEST['change_min_ability']))) {

        $ch = json_decode($min_ability, true);
        sort($ch);
        if (!in_array(@$_REQUEST['change_min_ability'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_min_ability'] => $_REQUEST['change_min_ability']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET min_ability = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
    if ((@$_REQUEST['selected_max_ability'] != '') and ( @$_REQUEST['change_max_ability'] != '') and ( is_numeric($_REQUEST['change_max_ability']))) {

        $ch = json_decode($max_ability, true);
        sort($ch);
        if (!in_array(@$_REQUEST['change_max_ability'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_max_ability'] => $_REQUEST['change_max_ability']));
            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET max_ability = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
}

function set_chnage_gear() {
    global $db;
    $sql = 'SELECT * FROM ' . VARIABLES_TABLE;
    $result = $db->sql_query($sql);
    ...
    if ((@$_REQUEST['selected_gear'] != '') and ( @$_REQUEST['change_gear'] != '')and ( !is_numeric($_REQUEST['change_gear']))) {
        $ch = json_decode($gear, true);
        if (!in_array(@$_REQUEST['change_gear'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_gear'] => $_REQUEST['change_gear']));




            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET gear = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
...
function set_chnage_spell() {
    ...
    if ((@$_REQUEST['selected_spell'] != '') and ( @$_REQUEST['change_spell'] != '')and ( !is_numeric($_REQUEST['change_spell']))) {
        $ch = json_decode($spell, true);
        if (!in_array(@$_REQUEST['change_spell'], $ch)) {
            $changed = array_replace($ch, array($_REQUEST['selected_spell'] => $_REQUEST['change_spell']));

            $sql1 = 'UPDATE ' . VARIABLES_TABLE . " SET spell = '" . json_encode($changed, JSON_FORCE_OBJECT) . "'";
            $db->sql_query($sql1);
        }
    }
}
