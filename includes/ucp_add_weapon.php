...
if (@$_REQUEST['seleted'] != "" && @$_REQUEST['seleted'] && @$_REQUEST['AddWaepen']) {
    ...
    echo "<input type=button name=addfields value=Add Weapon Fields class='button1' onclick='addweaponfields()'>";
}
...
if (@$_REQUEST['addfieldsweaponname'] != "" && @$_REQUEST['addfieldsweaponname'] && @$_REQUEST['Database'] && $_REQUEST['user_id']) {
 ...
 print_r($type);
    ...
    for ($i = 0; $i < $fields_count; $i++) {
        if($weapon_field[$i][0] == $addfieldsweaponname[$i])
        {
            $weapon_field[$i] = array($addfieldsweaponname[$i], array($attack[$i], $damage[$i], $critical_multiplier[$i], $range[$i], $type[$i]));
            //$fields[$i] = array($addfieldsweaponname[$i], array($attack[$i], $damage[$i], $critical_multiplier[$i], $range[$i], $type[$i]));
        }
           
    }
    print_r($weapon_field);
    ...
    echo $update = "UPDATE " . USER_VARIABLES_TABLE . " SET weapon_field='" . $json_encoded_string . "' WHERE user_id =" . $_REQUEST['user_id'];
    //mysql_query($update);
}

if ($_REQUEST['weponaname']) {
    $select = 'SELECT weapon_name FROM ' . USER_VARIABLES_TABLE . ' WHERE user_id =' . $_REQUEST['user_id'];
    $result1 = mysql_query($select);
    $row = mysql_fetch_assoc($result1);
    ...
    $update = "UPDATE " . USER_VARIABLES_TABLE . " SET weapon_name='" . $string . "' WHERE user_id =" . $_REQUEST['user_id'];
    mysql_query($update);
}
if ($_REQUEST['DeleteWeapen']) {

    $select = 'SELECT * FROM ' . USER_VARIABLES_TABLE . ' WHERE user_id =' . $_REQUEST['user_id'];
    $result1 = mysql_query($select);
