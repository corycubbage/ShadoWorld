...
for ($al = 0; $al < $alias_count; $al++) {
	...
	else
	{
        ...
        //$selected_Sig = getAliasSig ($selected_Sigdata, $selected_Sig_bbcode, $selected_Sig_bitfield,$config['allow_sig_bbcode'], $config['allow_sig_links'], $config['allow_sig_smilies']);
        ...
        //$parse_sig = new parse_message($selected_Sigdata);
        //$parse_sig->bbcode_uid = $selected_Sig_bbcode;
...
//debug_to_console ("Post Mode: " . $mode);
...
if ($submit || $preview || $refresh)
{
	...
	*/
	...
	{
                                                                                                                        // This is where we submit the messages CVC 11/25/15
            if ($submit)
		{
			debug_to_console("Submit == TRUE");
                        // Lock/Unlock Topic
            ...
            $sel = 'SELECT * FROM ' . USERS_TABLE . ' WHERE user_id=' . $poster_id;
            $result = $db->sql_query($sel);
            $topic_row = $db->sql_fetchrow($result);
			$md_myupdate=md5($myupdate);			
			$data = array(
				...
				'poster_id'			=> (int) $poster_id,                                //$post_data['poster_id'],   // CVC 11/26/15
                                'poster_name'                   => (string) $poster_name,                          //CVC 11/26/15
                                'user_colour'                   => $updated_color,                                  //CVC 11/26/15
				...
				'post_edit_reason'		=> $post_data['post_edit_reason'],
                                'post_edit_user'		=> ($mode == 'edit') ? $poster_id : ((isset($poster_name)) ? (int) $poster_name : 0),	
                ...
                //if ($mode == 'edit') {
                    $dmessage = $message_parser->message;                                           //Updated 12/02/15 to fix the blank edit issue.
                    $dmessage = str_replace("'", "\\'", $dmessage);                                 //Updated 12/02/15 to fix the blank edit issue.                        
                    //$sql = 'UPDATE ' . POSTS_TABLE . ' SET post_text = \'' . $dmessage . '\', post_checksum = "' . md5($message_parser->message) . '" WHERE post_id = ' . $data['post_id'];
                    //$db->sql_query($sql);
                //}
			...
			//$post_author_name = ((!$user->data['is_registered'] || $mode == 'edit') && $post_data['username'] !== '') ? $post_data['username'] : '';   // CVC 11/26/15
                        $post_author_name = ((!$user->data['is_registered'] || $mode == 'edit') && $poster_name !== '') ? $post_data['username'] : '';
			...
			*/
			...
			if ((!$auth->acl_get('f_noapprove', $data['forum_id']) && empty($data['force_approved_state'])) || (isset($data['force_approved_state']) && !$data['force_approved_state']))
			{
				...
				//$message .= (($user->data['user_id'] == ANONYMOUS) ? '' : ' '. $user->lang['POST_APPROVAL_NOTIFY']);  //CVC112615
                                $message .= (($data['poster_id'] == ANONYMOUS) ? '' : ' '. $user->lang['POST_APPROVAL_NOTIFY']);
...
// Load all variables for user to modify in posting_editor.html
...
$template->assign_vars(array(
    ...
    'S_ATTACK' => $attack,
    'S_DAMAGE' => $damage,
    'S_CRITICAL_MULTIPLIER' => $s_critical_multiplier,
    'S_RANGE' => $off_range,
    'S_TYPE' => $s_type,
    ...
    'MODE' => $modes,
    'FORUM' => $forums,
    'POST_SPELL_ID' => $post_id_spell,
    'S_SPELL_PAGE_OPTIONS' => $s_spell_page_spell,
    'S_T_VARIABLE' => $tvariable,
    'STATUS' => $status_value,
...
$page_data = array(
	...
	'S_HIT_CURRENT_COUNT' => $s_hit_current_data,
        ...
        'S_GEAR' => $s_gear_data,
        'S_NON_LETHAL' => $s_non_lethal_data,
        'S_VARIABLE_LEVEL' => $variable_level,                                  //CVC - 11/28/15 - YES/NO value on whether or not there are spells present on this character.
        'S_VARIABLE_ABILITY' => $variable_ability,
        'S_EDIT_LEVEL' => $selectbox_level,
        'S_EDIT_MIN_LEVEL' => $selectbox_level_min,
        'S_EDIT_MAX_LEVEL' => $selectbox_level_max,
