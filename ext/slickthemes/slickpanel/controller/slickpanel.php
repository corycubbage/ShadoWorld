...
class slickpanel {
	...
	public function __construct(\phpbb\auth\auth $auth, \phpbb\config\config $config, \phpbb\template\template $template, \phpbb\user $user, \phpbb\controller\helper $helper, $root_path, $php_ext, \slickthemes\slickpanel\controller\userconfig $userconfig){
		$this->auth = $auth;
		$this->config = $config;
